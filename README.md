...
系统首次运行时会自动创建SQLite数据库和默认用户账户：

- **默认管理员账户**：
  - 用户名：admin
  - 密码：admin123

> **数据库文件 `driving_system.db` 会在首次运行时自动创建在项目根目录。**
...
- 👤 **用户管理**：首次访问需要登录，支持注册新用户，每个用户有独立的个性化设置
...
- ⚙️ **个性化设置**：在用户中心管理个人资料、通知设置、隐私权限等。
